...
| 能力 | 说明 |
|---|---|
| **预约 → 面试** | 填简历 + 岗位 JD + 目标公司 → 立即生成一场**完整、有针对性**的面试计划（intro→behavioral→technical→coding→wrap，难度递进） |
| **真实语音面试** | 候选人可**按住说话**（WebAudio 采集 → Volcengine 实时 STT → DeepSeek 大脑 → MiniMax 语音合成回放）；也支持打字 |
| **手撕代码** | 到 coding 环节出现**力扣同款题**（本地题库）与代码编辑器；**AI 面试官实时读取并评判候选人的代码**（静态分析 + 提示阶梯），判对/错/部分/未完成 |
| **AI 看屏幕** | 候选人可"让面试官看我的屏幕" → Gemini 视觉理解和描述共享画面（AI 读视频流能力） |
| **信息充分** | Researcher 层在预约时检索（搜索引擎 + 牛客面经 + Tavily + 小红书/知乎[有 cookie]）→ 结构化成**岗位画像**（技能/考察点/公司流程/高频题），注入出题，问得比通用 AI 更准 |
| **可执行反馈** | `interviewer_os`：事后给出 **missing_slots → what_i_want_to_hear → 一句话改进建议**（带候选人原话证据），只在报告呈现，面试中不实时展示 |
| **转写 + 分享 + 录制** | 全程问答转写可下载；**公开只读分享页** `/share/{id}` 转发给他人评价；**语音回顾**把报告核心要点合成音频（录制） |

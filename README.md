...
| **预约 → 面试** | 填简历 + 公司/岗位/JD + 时间 + 是否含手撕 + 场景(含保研) + **面试官人格** + **面试方案(真实对话/按住说话/文字对话)** → 生成一场**真实、有时间概念**的面试 |
| **真实面试流程（核心）** | AI 面试官**真正地主持面试**，而非"后台搜题一个个丢给候选人"：约 60 分钟 = **40 分钟聊项目/经历/基础 + 20 分钟手撕代码**。开场让你**自我介绍 → 挑一个具体项目深挖 → 按你回答的长短与剩余时间，逐层追问项目细节/架构/难点/取舍 → 再结合岗位问基础与场景**。**每一轮都在后端重建一个 agent**，其 prompt 含：面试官身份(人格) + **你的简历** + **本场特殊要求(岗位/JD/场景/补充)** + **你和它全部的历史对话** + 时间预算 |

...
class LiveFlow:
    ...
    def _ask_agent(self) -> str:
        ...
        sys_ = (
            ...
            "【面试总体方向】自我介绍 → 介绍项目 → 提问项目细节 → 提问其他能力/知识 → "
            "提问对岗位的看法 → 手撕代码 → 收尾。这是你脑中自然的推进顺序，不是逐字脚本。\n"
            f"【当前大致阶段】{state}（下一阶段：{next_state}）。仅作方向参考，不要因此改变你的回应方式。\n"
            ...
            "   · 候选人沉默/只说\"不知道\" → 给一点引导或换个角度，别追问同样的东西。\n"
            "- 只在对话真正推进时自然过渡到下一个大方向（如项目聊透了再问基础），不要因为\"轮到下一阶段\"就生硬切换。\n"
            "- 一次只问一个问题；用中文口语化；不要复述问题、不要提评分、不要说\"我记录了\"。\n"
        )
        user = (
            "【以下是刚才的完整对话】\n" + self._history_block() +
            "\n\n【现在轮到你说话】请看着候选人最后一句回答，作为真人面试官说一句自然的回应/追问。"
            "只输出这句话本身，不要加引号或解释。"
        )
        try:
            llm = LLM()

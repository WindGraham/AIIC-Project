...
- **体验与打磨（commit 45–63）**：预约时**后台提前准备**（不再等 /start）+ 预约制（**尽快开始 / 30 分钟下限** + 黄红绿 **prep 灯**）；**面试官严厉度选项** + **完整面试总结文档**；语音全双工打磨（**可打断 / 可靠自动播放 / IME 修复 / ws 可达 URL**）+ **候选人语音气泡 / loading / 右侧布局**；**CodeMirror 6 代码编辑器** + 提交代码入聊天；**设备自测房间**（语音↔文本↔TTS + 实时看屏 + 搜索/LLM 接口测试）；**默认简历/公司管理** + 预约可选 + 搜索/屏幕等修复。
- **过程原则**：每阶段**真实 API 测试 + 对抗性审查 + 独立提交**（共 **63 个 commit**，commit 粒度清晰、便于回滚）。
